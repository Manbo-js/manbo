...
class Shard extends EventEmitter {
    ...
    createGuild(_guild) {
        ...
        if (
            this._client.shards.options.getAllUsers &&
            guild.members.size < guild.memberCount
        ) {
            this.getGuildMembers(guild.id, {
                presences:
                    this._client.shards.options.intents &&
                    this._client.shards.options.intents &
    ...
    disconnect(options = {}, error) {
            ...
            try {
                if (options.reconnect && this.sessionID) {
                    if (this.ws.readyState === WebSocket.OPEN) {
                        this.ws.close(4901, "Manbo: reconnect");
                    } else {
                        this.emit(
                            "debug",
                    ...
                    this.ws.close(1000, "Manbo: normal");
                }
            } catch (err) {
        ...
        if (
            this.sessionID &&
            this.connectAttempts >=
                this._client.shards.options.maxResumeAttempts
        ) {
            this.emit(
                "debug",
        ...
        if (
            ...
            this._client.shards.options.autoreconnect
        ) {
            /**
    ...
    initializeWS() {
        ...
        this.ws = new WebSocket(
            this._client.gatewayURL,
            this._client.shards.options.ws
        );
    ...
    onPacket(packet) {
        if (
            ...
            this._client.listeners("rawWS").length
        ) {
            /**
        ...
        switch (
            ...
            case "AUTO_MODERATION_ACTION_EXECUTION": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in AUTO_MODERATION_ACTION_EXECUTION`
                    );
                    break;
                }
            ...
            case "AUTO_MODERATION_RULE_CREATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in AUTO_MODERATION_RULE_CREATE`
                    );
                    break;
                }
            ...
            case "AUTO_MODERATION_RULE_DELETE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in AUTO_MODERATION_RULE_DELETE`
                    );
                    break;
                }
            ...
            case "PRESENCE_UPDATE": {
                if (packet.d.user.username !== undefined) {
                    ...
                    if (!user || oldUser) {
                        user = this._client.users.update(
                            packet.d.user,
                            this._client
                        );
                        /**
            ...
            case "VOICE_STATE_UPDATE": {
                ...
                if (
                    ...
                    const voiceConnection = this._client.voiceConnections.get(
                        packet.d.guild_id
                    );
                    if (voiceConnection) {
                        if (packet.d.channel_id === null) {
                            this._client.voiceConnections.leave(
                                packet.d.guild_id
                            );
                        } else if (
            ...
            case "TYPING_START": {
                ...
                if (this._client.listeners("typingStart").length > 0) {
                    ...
                    this.emit(
                        "typingStart",
                        this._client.getChannel(packet.d.channel_id) || {
                            id: packet.d.channel_id,
                        },
                        this._client.users.get(packet.d.user_id) || {
                            id: packet.d.user_id,
                        },
                        member
                    );
            ...
            case "MESSAGE_CREATE": {
                ...
                if (channel) {
                    ...
                    this.emit(
                        "messageCreate",
                        channel.messages.add(packet.d, this._client)
                    );
                } else {
                    this.emit(
                        "messageCreate",
                        new Message(packet.d, this._client)
                    );
                }
                break;
            }
            case "MESSAGE_UPDATE": {
                ...
                this.emit(
                    "messageUpdate",
                    channel.messages.update(packet.d, this._client),
                    oldMessage
                );
                break;
            }
            ...
            case "GUILD_CREATE": {
                if (!packet.d.unavailable) {
                    ...
                    this.emit(
                        ...
                        this._client.unavailableGuilds.add(
                            packet.d,
                            this._client
                        )
                    );
            ...
            case "GUILD_UPDATE": {
                ...
                this.emit(
                    "guildUpdate",
                    this._client.guilds.update(packet.d, this._client),
                    oldGuild
                );
                break;
            }
            case "GUILD_DELETE": {
                ...
                if (voiceConnection) {
                    if (voiceConnection.channelID) {
                        this._client.leaveVoiceChannel(
            ...
            case "GUILD_BAN_ADD": {
                ...
                this.emit(
                    "guildBanAdd",
                    this._client.guilds.get(packet.d.guild_id),
                    this._client.users.update(packet.d.user, this._client)
                );
                break;
            }
            ...
            case "CHANNEL_CREATE": {
                ...
                if (packet.d.guild_id) {
                    if (!channel.guild) {
                        channel.guild = this._client.guilds.get(
                            packet.d.guild_id
                        );
                        if (!channel.guild) {
                            this.emit(
                    ...
                    this._client.channelGuildMap[packet.d.id] =
                        packet.d.guild_id;
                    /**
            ...
            case "CHANNEL_UPDATE": {
                ...
                if (oldType === packet.d.type) {
                    ...
                    if (packet.d.guild_id) {
                        ...
                        this._client.privateChannels.add(
                            newChannel,
                            this._client
                        );
                    } else {
                 ...
                 * @prop {Array<Object>} oldChannel.availableTags An available tags of the channel (forum channels only)
                 ...
                 * @prop {Object} oldChannel.defaultReactionEmoji The default emoji to show as the reaction button
            ...
            case "CHANNEL_DELETE": {
                if (
                    ...
                    if (this.id === 0) {
                        const channel = this._client.privateChannels.remove(
                            packet.d
                        );
                        if (channel) {
                            delete this._client.privateChannelMap[
                ...
                if (!guild) {
                    this.emit(
                        ...
                        `Received GUILD_MEMBERS_CHUNK, but guild ${packet.d.guild_id} is ` +
                            (this._client.unavailableGuilds.has(
                                packet.d.guild_id
                            )
                                ? "unavailable"
                                : "missing"),
            ...
            case "READY": {
                ...
                this._client.user = this._client.users.update(
                    new ExtendedUser(packet.d.user, this._client),
                    this._client
                );
                ...
                packet.d.guilds.forEach((guild) => {
                    if (guild.unavailable) {
                        ...
                        this._client.unavailableGuilds.remove(
                ...
                packet.d.private_channels.forEach((channel) => {
                    if (
                        ...
                        this._client.privateChannelMap[
                ...
                if (
                    this._client.unavailableGuilds.size > 0 &&
            ...
            case "THREAD_CREATE": {
                ...
                const parent = this._client.getChannel(packet.d.parent_id);
                ...
                if (!channel) {
                    ...
                    this.emit(
                        ...
                        this._client.guilds
                            ...
                            .threads.add(thread, this._client),
                        null
                    );
                    this._client.threadGuildMap[packet.d.id] =
                        packet.d.guild_id;
                    break;
                }
            ...
            case "THREAD_LIST_SYNC": {
                ...
                const activeThreads = packet.d.threads.map((t) =>
                    guild.threads.update(t, this._client)
                );
                const joinedThreadsMember = packet.d.members.map((m) =>
                    guild.threads.get(m.id).members.update(m, this._client)
                );
                /**
            ...
            case "STAGE_INSTANCE_CREATE": {
                ...
                this.emit(
                    "stageInstanceCreate",
                    guild.stageInstances.add(packet.d, this._client)
                );
                break;
            }
            case "STAGE_INSTANCE_UPDATE": {
                ...
                this.emit(
                    "stageInstanceUpdate",
                    guild.stageInstances.update(packet.d, this._client),
                    oldStageInstance
                );
                break;
            }
            case "STAGE_INSTANCE_DELETE": {
                ...
                if (!guild) {
                    this.emit(
                        "stageInstanceDelete",
                        new StageInstance(packet.d, this._client)
                    );
                    break;
                }
                /**
            ...
            case "GUILD_SCHEDULED_EVENT_CREATE": {
                ...
                if (!guild) {
                    this.emit(
                        ...
                        new GuildScheduledEvent(packet.d, this._client)
                    );
                    break;
                }
                ...
                this.emit(
                    "guildScheduledEventCreate",
                    guild.events.add(packet.d, this._client)
                );
                break;
            }
            case "GUILD_SCHEDULED_EVENT_UPDATE": {
                ...
                this.emit(
                    "guildScheduledEventUpdate",
                    guild.events.update(packet.d, this._client),
                    oldEvent
                );
                break;
            }
            ...
            case "GUILD_SCHEDULED_EVENT_USER_ADD": {
                ...
                if (!guild) {
                    this.emit(
                        ...
                        {
                            id: packet.d.guild_scheduled_event_id,
                            guild: { id: packet.d.guild_id },
                        },
                        user
                    );
            ...
            case "INTERACTION_CREATE": {
                ...
                this.emit(
                    "interactionCreate",
                    Interaction.from(packet.d, this._client)
                );
                break;
            }
