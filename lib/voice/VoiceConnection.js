...
class VoiceConnection extends EventEmitter {
        ...
        if (!Sodium && !NaCl) {
            try {
                ...
                try {
                    ...
                    throw new Error(
                        "Error loading tweetnacl/libsodium, voice not available"
                    );
                }
            }
        }
    ...
    connect(data) {
        ...
        this.connectionTimeout = setTimeout(
            ...
            this.shard._client
                ? this.shard._client.shards.options.connectionTimeout
                : 30000
        ).unref();
        if (!data.endpoint) {
        ...
        if (!data.token || !data.session_id || !data.user_id) {
            this.disconnect(
                new Error(
                    "Malformed voice server update: " + JSON.stringify(data)
                )
            );
            return;
        }
        ...
        this.ws.on("message", (m) => {
            ...
            switch (packet.op) {
                case VoiceOPCodes.READY: {
                    ...
                    this.emit(
                        "debug",
                        "Connecting to UDP: " + this.udpIP + ":" + this.udpPort
                    );

                    this.udpSocket = Dgram.createSocket(
                        Net.isIPv6(this.udpIP) ? "udp6" : "udp4"
                    );
                    ...
                    this.udpSocket.once("message", (packet) => {
                        ...
                        const localPort = packet.readUInt16BE(
                            packet.length - 2
                        );
                        this.emit(
                            "debug",
                            `Discovered IP: ${localIP}:${localPort} (${packet.toString(
                                "hex"
                            )})`
                        );
                ...
                case VoiceOPCodes.SPEAKING: {
                     ...
                     */
                    this.emit(
                        packet.d.speaking ? "speakingStart" : "speakingStop",
                        packet.d.user_id
                    );
                    break;
                }
    ...
    play(source, options = {}) {
        ...
        options.voiceDataTimeout = !isNaN(options.voiceDataTimeout)
            ? options.voiceDataTimeout
            : 2000;
        ...
        options.frameSize =
            options.frameSize ||
            (options.samplingRate * options.frameDuration) / 1000;
        options.pcmSize =
    ...
    registerReceiveEventHandler() {
        this.udpSocket.on("message", (msg) => {
            ...
            if (Sodium) {
                data = Buffer.alloc(
                    msg.length - 12 - Sodium.crypto_secretbox_MACBYTES
                );
            ...
            if (this.receiveStreamOpus) {
                 ...
                 */
                this.receiveStreamOpus.emit(
                    "data",
                    data,
                    this.ssrcUserMap[nonce.readUIntBE(8, 4)],
                    nonce.readUIntBE(4, 4),
                    nonce.readUIntBE(2, 2)
                );
            }
            if (this.receiveStreamPCM) {
                const userID = this.ssrcUserMap[nonce.readUIntBE(8, 4)];
                if (!this.opus[userID]) {
                    this.opus[userID] = createOpus(
                        this.samplingRate,
                        this.channels,
                        this.bitrate
                    );
    ...
    resume() {
        ...
        if (this.current) {
            this.setSpeaking(1);
            if (this.current.pausedTimestamp) {
                this.current.pausedTime +=
                    Date.now() - this.current.pausedTimestamp;
    ...
    _send() {
        ...
        if ((this.current.buffer = this.piper.getDataPacket())) {
            ...
            this.current.bufferingTicks <
                this.current.options.voiceDataTimeout /
                    (4 * this.current.options.frameDuration)
        ) {
            // wait for data
            ...
            this.timestamp =
                (this.timestamp + 3 * this.current.options.frameSize) >>> 0;
        ...
        this.sendAudioFrame(
            this.current.buffer,
            this.current.options.frameSize
        );
    ...
    _sendAudioFrame(frame) {
        ...
        if (Sodium) {
            ...
            NaCl.lowlevel.crypto_secretbox(
                this.sendBuffer,
                this.sendBuffer.subarray(BOXZEROBYTES),
                ZEROBYTES + frame.length,
                this.sendNonce,
                this.secret
            );
            this.sendNonce.copy(this.sendBuffer, BOXZEROBYTES - 12, 0, 12);
            return this.sendUDPPacket(
                this.sendBuffer.subarray(
                    BOXZEROBYTES - 12,
                    BOXZEROBYTES + length
                )
            );
