...
/**
 * Represents a browser's websocket usable by Manbo
...
class BrowserWebSocket extends EventEmitter {
    constructor(url) {
        ...
        if (typeof window === "undefined") {
            throw new Error(
                "BrowserWebSocket cannot be used outside of a browser environment"
            );
        }
        ...
        this._ws.onerror = (event) =>
            this.emit(
                "error",
                new BrowserWebSocketError("Unknown error", event)
            );
