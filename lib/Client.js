...
/**
 * Represents the main Manbo client
 ...
 * @prop {Object} options Manbo options
 ...
 * @prop {Collection<Shard>} shards Collection of shards Manbo is using
...
class Client extends EventEmitter {
     ...
     * @arg {Object} options Manbo client options
     ...
     * @arg {Boolean} [options.autoreconnect=true]  [DEPRECATED] Have Manbo autoreconnect when connection is lost. This option has been moved under `options.gateway`
     ...
     * @arg {Object} [options.disableEvents]  [DEPRECATED] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/Manbo/docs/reference#ws-event-names). This option has been moved under `options.gateway`
     ...
     * @arg {Boolean} [options.gateway.autoreconnect=true] Have Manbo autoreconnect when connection is lost
     ...
     * @arg {Object} [options.gateway.disableEvents] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/Manbo/docs/reference#ws-event-names)
     ...
     * @arg {Number | Array<String | Number>} [options.gateway.intents] A list of [intent names](/Manbo/docs/reference), pre-shifted intent numbers to add, or a raw bitmask value describing the intents to subscribe to. Some intents, like `guildPresences` and `guildMembers`, must be enabled on your application's page to be used. By default, all non-privileged intents are enabled.
     ...
     * @arg {Number | String} [options.gateway.maxConcurrency=1] The number of shards that can start simultaneously. If "auto" Manbo will use Discord's recommended shard concurrency.
     * @arg {Number | String} [options.gateway.maxShards=1] The total number of shards you want to run. If "auto" Manbo will use Discord's recommended shard count.
     ...
     * @arg {Number | Array<String | Number>} [options.intents] [DEPRECATED] A list of [intent names](/Manbo/docs/reference), pre-shifted intent numbers to add, or a raw bitmask value describing the intents to subscribe to. Some intents, like `guildPresences` and `guildMembers`, must be enabled on your application's page to be used. By default, all non-privileged intents are enabled. This option has been moved under `options.gateway`
     ...
     * @arg {Number} [options.latencyThreshold=30000] [DEPRECATED] The average request latency at which Manbo will start emitting latency errors. This option has been moved under `options.rest`
     ...
     * @arg {Number | String} [options.maxShards=1] The total number of shards you want to run. If "auto" Manbo will use Discord's recommended shard count. This option has been moved under `options.gateway`
     ...
     * @arg {Number} [options.rest.latencyThreshold=30000] The average request latency at which Manbo will start emitting latency errors
    ...
    addMessageReaction(channelID, messageID, reaction, userID) {
        if (userID !== undefined) {
            ...
            this.emit(
                "warn",
                '[DEPRECATED] addMessageReaction() was called without an "@me" `userID` argument'
            );
        }
        ...
        return this.requestHandler
            ...
            .then((applicationCommands) =>
                applicationCommands.map(
                    (applicationCommand) =>
                        new ApplicationCommand(applicationCommand, this)
                )
            );
    }
    ...
    async connect() {
        ...
        try {
            const data = await (this.shards.options.maxShards === "auto" ||
            (this.shards.options.shardConcurrency === "auto" && this.bot)
                ? this.getBotGateway()
                : this.getGateway());
            if (
                !data.url ||
                (this.shards.options.maxShards === "auto" && !data.shards)
            ) {
            ...
            if (
                this.shards.options.shardConcurrency === "auto" &&
                data.session_start_limit &&
                typeof data.session_start_limit.max_concurrency === "number"
            ) {
                this.shards.options.maxConcurrency =
                    data.session_start_limit.max_concurrency;
            }

            for (
                let i = this.shards.options.firstShardID;
                i <= this.shards.options.lastShardID;
                ++i
            ) {
                this.shards.spawn(i);
            }
            ...
            const reconnectDelay = this.shards.options.reconnectDelay(
    ...
    createAutoModerationRule(guildID, options) {
        return this.requestHandler.request(
            "POST",
            Endpoints.AUTO_MODERATION_RULES(guildID),
            true,
            {
     ...
     * @arg {Array<Object>} [options.availableTags] An available tags for a forum channel 
     ...
     */
    createGuildForumThread(channelID, options) {
        if (!options.message) {
            options.message = {};
        }
        ...
        if (options.message !== undefined) {
            if (options.message.allowedMentions) {
                options.message.allowedMentions = this._formatAllowedMentions(
                    options.message.allowedMentions
                );
            }
            if (options.message.embed) {
                if (!options.message.embeds) {
                    options.message.embeds = [];
                }
                options.message.embeds.push(options.message.embed);
            }
        }
        ...
        return this.requestHandler
            .request(
                "POST",
                Endpoints.THREAD_IN_FORUM(channelID),
                true,
                {
     ...
     * @arg {Array<String>} [options.appliedTags] An array of applied tag IDs for the thread (forum posts only)
     ...
     * @arg {Number?} [options.defaultAutoArchiveDuration] The default duration of newly created threads in minutes to automatically archive the thread after inactivity (60, 1440, 4320, 10080) (forum channels only)
     * @arg {Object} [options.defaultReactionEmoji] The emoji to show as the reaction button (forum channels only)
    ...
    /**
     * Edit the guild mfa level
     * @arg {Object} options The options to edit the MFA level
     * @arg {Boolean} [options.level] The guild mfa level to set, either 0 (None) or 1 (Elevated)
     * @arg {String?} [options.reason] The reason to be displayed in audit logs
    ...
    getCommands(withLocalizations) {
        ...
        return this.requestHandler
            .request(
                "GET",
                Endpoints.COMMANDS(this.application.id) + (qs ? "?" + qs : ""),
                true
            )
            .then((applicationCommands) =>
                applicationCommands.map(
    ...
    getGuildScheduledEvents(guildID, options = {}) {
        ...
        return this.requestHandler
            .request(
                "GET",
                Endpoints.GUILD_SCHEDULED_EVENTS(guildID),
                true,
                options
            )
            .then((data) =>
                data.map((event) => new GuildScheduledEvent(event, this))
            );
    }
                ...
                data.map((eventUser) => {
                    ...
                    return {
                        guildScheduledEventID:
                            eventUser.guild_scheduled_event_id,
                        member:
    ...
    getRESTChannel(channelID) {
        if (!this.options.restMode) {
            return Promise.reject(new Error("Manbo REST mode is not enabled"));
        }
        return this.requestHandler
